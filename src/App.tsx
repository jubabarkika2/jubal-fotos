...
import { initAuth, googleSignIn, googleSignInRedirect, checkRedirectResult, logout } from './firebase';
...
export default function App() {
  ...
  useEffect(() => {
    // Primeira verificação de retorno de redirecionamento (fluxo celular / tablet)
    checkRedirectResult()
      .then((result) => {
        if (result) {
          setUser(result.user);
      ...
      .catch((err) => {
        console.error("Erro capturando redirecionamento do Google:", err);
      });
  ...
  // Função para fazer login com Google (Popup)
  const handleLogin = async () => {
    ...
    try {
      ...
      console.error('Falha de login por popup:', err);
      setPhotoState(prev => ({
        ...prev,
        error: 'Erro de autenticação para acessar seu Gmail. Se estiver no celular, use o botão "Entrar com celular".',
      }));
    } finally {
  ...
  // Função para fazer login com Google (Redirect - celular)
  const handleLoginRedirect = async () => {
    ...
    try {
      ...
      console.error('Falha de login por redirect:', err);
      setPhotoState(prev => ({
        ...prev,
        error: 'Erro ao redirecionar para login do Google. Tente abrir o site direto em seu navegador externo.',
      }));
      setIsLoggingIn(false);
    }
  };
        ...
        <main className="flex flex-1 items-center justify-center px-4 py-16">
          <div className="w-full max-w-md rounded-3xl border border-slate-100 bg-white p-8 shadow-xl text-center">
            ...
            <div className="mt-8 space-y-4">
              {/* Botão Oficial Google (Popup - Computador) */}
                  ...
                  <div className="gsi-material-button-content-wrapper">
                    {isLoggingIn ? (
                      ...
                      <>
                        ...
                        <span className="gsi-material-button-contents">Entrar no Computador (Popup)</span>
                      </>
                    )}
                  </div>
                </button>
                <span className="mt-1 flex items-center gap-1 text-[10px] text-slate-400 font-medium">
                  <Laptop className="h-3 w-3" /> Abre uma janela de login flutuante.
                </span>
              </div>
              ...
              <div className="relative flex py-2 items-center">
                ...
                <span className="flex-shrink mx-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">ou no celular</span>
                <div className="flex-grow border-t border-slate-100"></div>
              </div>

              {/* Botão Celular (Redirect) */}
              <div className="flex flex-col items-center">
                <button
                  id="google-signin-redirect-btn"
                  onClick={handleLoginRedirect}
                  disabled={isLoggingIn}
                  className="flex w-full items-center justify-center gap-2.5 rounded-full border border-indigo-100 bg-indigo-50/55 py-3 px-6 text-xs font-bold text-indigo-700 transition duration-200 hover:bg-indigo-50 hover:border-indigo-200 disabled:opacity-50 cursor-pointer"
                >
                  <Smartphone className="h-4 w-4" />
                  Entrar no Celular (Sem Popups)
                </button>
                <div className="mt-2 text-[10px] text-slate-400 leading-relaxed max-w-xs mx-auto">
                  Redireciona a própria página para o Google de forma segura. 
                  <span className="font-semibold text-slate-500"> Evita bloqueios de tela em celulares!</span>
                </div>
              </div>
            </div>
